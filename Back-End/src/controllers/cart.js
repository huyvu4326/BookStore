...
export const addItemToCart = async (req, res) => {
    try {
        const { productId, imageUrl, name, promotionalPrice, originalPrice, author } = req.body
        const user = req.user
        const cart = await Cart.findOne({ userId: user._id })
        if (!cart) {
            const cartNew = await Cart.create({ userId: user._id })
            cartNew.items.push({ productId, imageUrl, name, promotionalPrice, originalPrice, author })
            ...
            if (itemExist) {
                ...
                cart.items.push({ productId, imageUrl, name, promotionalPrice, originalPrice, author })
                cart.total = cart.items.reduce((accumulator, currentValue) => accumulator + currentValue.quantity * currentValue.productPrice, 0)
                await cart.save()
                return res.status(200).json(cart)
            }
